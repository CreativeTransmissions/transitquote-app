...
- **`apiClient` recovers the JSON envelope from leaked PHP warnings/errors** via `transformResponse` (`parseApiBody`). The backend has prepended PHP noise to REST JSON more than once — a `Deprecated` warning on `/configuration` and `/jobs` (fixed 2026-06-01), then a `$journey_order` deprecation **+** a `wpdberror` SQL block on `GET /jobs?id=` (regressed 2026-06-02, re-fixed same day). **Do NOT assume the body is always clean JSON** — this guard is **permanent defence-in-depth**, not a temporary workaround. `parseApiBody` scans for the *trailing* JSON envelope (skipping stray brackets inside the noise), so it survives the SQL-error case that broke the old first-bracket slice. (See `docs/API_NOTES.md` §5.)
...
## Windows Emulator Dev Loop (read before touching the emulator)

These gotchas cost real time in past sessions. Skills automate the loop:
`refresh-app` (rebuild + show), `android-shot` (screenshot), `inspect-device-db` (SQLite/outbox).

- **Run commands from `TQApp/`.** The Claude session cwd is the *parent*
  `C:/Users/andre/AndroidStudioProjects`. Always `cd .../TQApp` first or you get
  "File does not exist (cwd is AndroidStudioProjects)".
- **`export MSYS_NO_PATHCONV=1`** at the front of any line with `adb` + a device path
  (`/sdcard/...`). Git-Bash otherwise rewrites it to `C:/Program Files/Git/sdcard/...` →
  `failed to stat remote object`.
- **Fast UI iteration = Metro Fast Refresh, NOT rebuilds.** This is a debug build with dev
  support on (no `getJSBundleFile()` override, no `expo-updates`/`expo-dev-client`), so the app
  loads JS **live from Metro when Metro is reachable**; the embedded
  `android/app/src/main/assets/index.android.bundle` is only a **fallback** for when Metro is
  unreachable. The reason JS edits sometimes "don't appear" is that **Avast Web Shield blocks the
  emulator→Metro path** — the app then falls back to the stale embedded bundle. Fix Metro, don't
  rebuild: Avast Web Shield OFF → `npx expo start` → `adb reverse tcp:8081 tcp:8081` → relaunch →
  edits Fast-Refresh on save.
- **Do NOT delete the embedded bundle.** `android/` is fully gitignored (regenerated by
  ...
  (no `expo-dev-client`; `clearState` wipes the dev-server URL; Avast blocks Metro — see
  docs/SMOKE_TESTING.md §2). Regenerate it only for the rebuild path:
  `NODE_TLS_REJECT_UNAUTHORIZED=0 npx expo export:embed --platform android --dev false --entry-file index.ts --bundle-output android/app/src/main/assets/index.android.bundle --assets-dest android/app/src/main/res`
  then `:app:assembleDebug` → `adb install -r` → relaunch. Verify the bundle contains your change
  (`grep` a token) before screenshotting. Use this path only for Maestro / standalone / device.
- **Screenshots:** use `adb shell screencap -p /sdcard/shot.png` + `adb pull`, NOT
  `adb exec-out screencap` (returns stale/0-byte frames here).
- **On-device SQLite:** copy the DB off-device (`run-as ... cat > /sdcard/...` then pull) and
  query on the host. Never run inline `sqlite3` over `adb shell` — it isn't on the device and
  the quoting fails.
- **DDEV bridge:** to reach `tq-pro-teams-php8.ddev.site` from the emulator, run
  `scripts/emulator-bridge.ps1` (sets `adb reverse`) — don't hand-roll it. HTTPS needs the
  mkcert truststore: gradle with
  `JAVA_TOOL_OPTIONS="-Djavax.net.ssl.trustStore=$USERPROFILE/.tqapp-certs/cacerts -Djavax.net.ssl.trustStorePassword=changeit"`.
- **Long builds:** run `assembleDebug` / metro waits with `run_in_background` and let the
  harness notify you — don't foreground `sleep`/poll loops.
- **Read before Edit/Write** — Edit/Write on an unread file fails; Read it first.
